...
class GuidePDF(FPDF):
    ...
    def axe_banner(self, n: int, title: str, subtitle: str):
        ...
        self.cell(0, 6, f"  {subtitle}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.set_text_color(*BLACK)
        self.ln(3)
    ...
    def code_block(self, lines: list, title: str = ""):
        ...
        for line in lines:
            ...
            if line.strip().startswith("#"):
                ...
                line.strip().startswith(k) for k in ("python ", "pip ", "git ", "cd ")
            ):
...
def build_pdf(output_path: str):
    ...
    pdf.body_text(
        ...
        size=9,
        link="https://convokit.cornell.edu/documentation/winning.html",
    )
    ...
    pdf.section_header(
        "Approche A - Features stylistiques pairwise  [RECOMMANDEE CPU]", ACCENT
    )
    pdf.body_text(
    ...
    pdf.axe_banner(
        2,
        "DETECTION DE TEXTES SYNTHETIQUES",
    ...
    pdf.axe_banner(
        3,
        "GENERATION D'ARGUMENTS CONVAINCANTS",
    ...
    pdf.section_header(
        "Etape 1 - Fine-tuner le LLM sur les arguments gagnants", SUCCESS
    )
    pdf.body_text(
    ...
    pdf.section_header(
        "Etape 2b - Generation par Prompt Engineering (zero-shot)", SUCCESS
    )
    pdf.body_text(

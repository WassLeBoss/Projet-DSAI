...
def format_cmv_data(raw_df):
    ...
    for p_id in valid_pairs:
        ...
        if isinstance(op, pd.Series):
            op = op.iloc[0]
        if isinstance(winning_arg, pd.Series):
            winning_arg = winning_arg.iloc[0]
        if isinstance(losing_arg, pd.Series):
            losing_arg = losing_arg.iloc[0]
...
def find_best_threshold(model_path, csv_path):
    if not os.path.exists(model_path):
        ...
        print(
            "Veuillez télécharger le fichier 'best_roberta_cmv.pt' et le placer à la racine du projet."
        )
        return None
    ...
    for item in val_data:
        ...
        if item["label"] == 0:
            single_val_data.append(
                {"op": op, "arg": item["arg_0"], "label": 1}
            )  # Gagnant (success=1)
            single_val_data.append(
                {"op": op, "arg": item["arg_1"], "label": 0}
            )  # Perdant (success=0)
        else:
            single_val_data.append(
                {"op": op, "arg": item["arg_0"], "label": 0}
            )  # Perdant (success=0)
            single_val_data.append(
                {"op": op, "arg": item["arg_1"], "label": 1}
            )  # Gagnant (success=1)
    ...
    print(
        f"[...] Chargement de RoBERTa sur {device} (ceci peut prendre quelques instants)..."
    )
    ...
    print(
        f"Seuil en probabilité (Sigmoïde) : {torch.sigmoid(torch.tensor(best_threshold)).item():.4f}"
    )
    ...
    meta_path = os.path.join(
        os.path.dirname(model_path), "roberta_persuasion_metadata.json"
    )
    metadata = {
        "optimal_threshold": float(best_threshold),
        "optimal_probability_threshold": float(
            torch.sigmoid(torch.tensor(best_threshold)).item()
        ),

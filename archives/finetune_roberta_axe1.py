...
def locate_dataset():
    ...
    raise FileNotFoundError(
        "Impossible de trouver WAC.csv ou WAC_final.csv dans les répertoires courants ou parents."
    )
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "attention_mask": torch.cat(
                [enc_0["attention_mask"], enc_1["attention_mask"]], dim=0
            ),
...
def main():
    ...
    print(
        f"Répartition : {len(train_data)} Train | {len(val_data)} Val | {len(test_data)} Test"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        print(
            f"Perte d'entraînement : {avg_train_loss:.4f} | Précision de Validation : {val_acc:.4f}"
        )
    ...
    print(
        classification_report(
            test_labels, test_preds, target_names=["Option 0 Gagne", "Option 1 Gagne"]
        )
    )

    ...
    def __init__(self, model_dir_or_file: str):
        ...
        if os.path.isdir(model_dir_or_file):
            ...
            self.meta_path = os.path.join(
                model_dir_or_file, "roberta_persuasion_metadata.json"
            )
        else:
            self.model_path = model_dir_or_file
            self.meta_path = os.path.join(
                os.path.dirname(model_dir_or_file), "roberta_persuasion_metadata.json"
            )
        ...
        if os.path.exists(self.model_path):
            ...
            raise FileNotFoundError(
                f"Fichier de modèle non trouvé à l'emplacement : {self.model_path}"
            )

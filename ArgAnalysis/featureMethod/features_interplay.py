from features_utils import tokenize, content_words, STOPWORDS

def overlap_features(reply_tokens: list[str], op_tokens: list[str], suffix: str):
    A = set(reply_tokens)
    O = set(op_tokens)
    inter = A & O
    union = A | O
...
def interplay_features(reply_text: str, op_text: str):
